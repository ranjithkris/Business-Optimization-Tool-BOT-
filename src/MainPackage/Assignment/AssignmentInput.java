...
public class AssignmentInput extends JFrame{
	...
	static JButton bAbout = new JButton(("About Assignment"));
	...
	JPanel inputPanel = new JPanel();
        JLabel personLabel[] = new JLabel[row_value];
        JLabel jobLabel[] = new JLabel[col_value];
	
	
	public AssignmentInput(int width,int height,int rowValue,int colValue)
        {
            
            row_value = rowValue;
            col_value = colValue;
            
        ImageIcon img=new ImageIcon(MainWindow.class.getResource("bot.png"));
        try
        {
        inputFrame.setIconImage(img.getImage());
        }
        catch(Exception e)
        {
            ...
            inputFrame.setTitle(("ASSIGNMENT PROBLEM"));
            ...
            {
		jobLabel[i]= new JLabel("Job "+(i+1),SwingConstants.CENTER);
                inputPanel.add(jobLabel[i],theLayout);
                theLayout.gridx+=1;
            }
		...
		for(int j=0;j<col_value;j++)
		{
            ...
            personLabel = new JLabel[row_value];
		...
		theLayout.gridy+=1;
                ...
                personLabel[i]= new JLabel("Person "+(i+1),SwingConstants.CENTER);
                inputPanel.add(personLabel[i],theLayout);
                theLayout.gridx+=1;
                    ...
                    {	
                        try
                        {
                            for(int i=0;i<row_value;i++)
                            {
                                for(int j=0;j<col_value;j++)
                                {
                                    cost[i][j] = Double.parseDouble(inputCost[i][j].getText());
                                }
                            }
                            ...
                            implementHungarrian.compute(cost, row_value, col_value);
                        }
                        catch(Exception e)
                        {
            ...
            bCompute.addKeyListener(new KeyListener()
            {
                ...
                public void keyPressed(KeyEvent e) 
                {
                    ...
                    if(e.getKeyCode() == KeyEvent.VK_ENTER)
                    {
                        ...
                        {
                            JOptionPane.showMessageDialog(null,"key1Enter numbers","ERROR",WARNING_MESSAGE);
                        }	
                }
            }
    			...
    			public void actionPerformed(ActionEvent ae)
                        {
                            ...
                            try {
                                    ...
                                    File file = new File(source+"\\html\\assignment.html");
                                    URI uri = file.toURI();
